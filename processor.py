            ...
            for tag in self.tags:
                ...
                if validator.is_valid(text_lines):
                    ...
                    if sentence_score > self.parameters['min_sent_score']:
                        ...
                        meta = {'tag': tag, 'attribute': item}
                        meta.update(validator.create_metadata(text_lines))
                        meta.update({'sentence_score': sentence_score})
                        metadata.append(meta)

        if len(metadata) > 0:
            metadata = sorted(metadata, key=lambda on: on['sentence_score'], reverse=True)

            tag = metadata[0].get('tag')
            attribute = metadata[0].get('attribute')
